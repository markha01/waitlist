...
export async function sendNotifyEmail(
  ...
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.warn("SMTP credentials not configured — skipping notify email.");
    return;
  }
  ...
  await transporter.sendMail({
    from,
    to: email,
    subject: `It's your turn at Liivo ✦`,
    html: buildNotifyEmailHtml(firstName),
    text: buildNotifyEmailText(firstName),
  });
}

function buildNotifyEmailHtml(firstName: string): string {
  const year = new Date().getFullYear();
  return `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>It's your turn at Liivo</title>
</head>
<body style="margin:0;padding:0;background-color:#0d0d0d;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#0d0d0d;padding:48px 16px;">
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0"
          ...
          <tr>
            <td style="padding:40px 40px 32px;">
              <h2 style="margin:0 0 18px;font-size:22px;font-weight:600;color:#ffffff;line-height:1.3;">
                It&apos;s your turn, ${firstName}!
              </h2>
              <p style="margin:0 0 16px;font-size:15px;color:#888;line-height:1.75;">
                The wait is over — your spot at Liivo is ready. We&apos;re excited to welcome you.
              </p>
              <p style="margin:0 0 28px;font-size:15px;color:#888;line-height:1.75;">
                Please get in touch with us to book your appointment at your earliest convenience.
              </p>
              ...
              <div style="background-color:#1a1a1a;border:1px solid #2a2a2a;border-left:3px solid #d4af70;border-radius:10px;padding:20px 24px;">
                ...
                <p style="margin:0;font-size:14px;color:#aaa;line-height:1.65;">
                  Reply to this email or reach out to us directly to lock in your time.
                </p>
              </div>
            </td>
          </tr>
...
function buildNotifyEmailText(firstName: string): string {
  const year = new Date().getFullYear();
  return `
It's your turn, ${firstName}!

The wait is over — your spot at Liivo Hair Salon is ready. We're excited to welcome you.
